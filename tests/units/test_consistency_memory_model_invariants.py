...
def test_memory_model_invariant_evaluation():
    meta, fp = _meta()
    ...
    # Currently memory model invariant check is a stub returning True
    assert "MEMORY_MODEL_INVARIANTS" not in report.failed_invariant_domains
    assert report.severity_classification == SystemHealthSeverity.SYSTEM_HEALTH_OK.value
