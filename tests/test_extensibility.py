...
def test_custom_constraint():
    ...
    assert constraint.validate(-1) == False
    assert constraint.validate(None) == False
...
def test_rule_registry():
    ...
    # Only rules with None heuristic apply (test_rule has heuristic that returns False)
...
def test_plugin_interface():
    ...
    class TestPlugin(PipelinePlugin):
        ...
        def get_hooks(self):
            return {
                HookPoints.PRE_PIPELINE: lambda ctx, **kw: None
            }
