...
from module_05_ir_normalization.ir_serialization import (
    IRArtifact, IRManifest, IRArtifactManager, IntegrityError,
    serialize_deterministically, compute_artifact_hash,
    verify_artifact_integrity, serialize_compressed, deserialize_compressed,
    validate_loaded_artifact, IREntityFactory
)
from module_05_ir_normalization.ir_entities import (
    InterfaceUnit, Endianness, ScalarType, ScalarKind, FunctionSymbol,
    CallingConvention, PointerType, ArrayType, ArrayKind, StructureType,
    FieldEntity, ReturnEntity, VariableSymbol, EntityKind, ParameterEntity
)
...
class TestEntityFactory:
    ...
    def test_reconstruct_struct(self):
        ...
        f = FieldEntity(field_index=0, field_name="a", type_reference="int", byte_offset=0, size_bytes=4)
        s.add_field(f)
        data = s.to_dict()
        res = IREntityFactory.from_dict(data)
