...
from module_05_ir_normalization.cli import (
    create_parser, OutputFormatter, __version__, main
)
...
class TestArgumentParser:
    ...
    @pytest.mark.parametrize("cmd, input_file", [
        ("normalize", "in.json"),
        ("validate", "art.json"),
        ("inspect", "art.json"),
    ])
    ...
    @pytest.mark.parametrize("flag, attr, val", [
...
class TestCommandLogic:
    ...
    def test_normalize_execution_success(self, mock_orch_cls, tmp_path):
        ...
        report = OrchestrationReport(validation_passed=True, output_artifact_path=str(tmp_path / "out.json"))
