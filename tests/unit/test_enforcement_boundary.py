...
from module_06_contract_schema.enforcement_boundary import (
    EnforcementMode, ViolationType, EnforcementViolation,
    EnforcementStats, LanguageAdapter, PythonAdapter, EnforcementEngine
)
...
class TestEnforcementStats:
    ...
    def test_report(self):
        stats = EnforcementStats()
        stats.record_call()
        stats.record_violation(EnforcementViolation("c1", ViolationType.SIZE, "e1", "1", "0", Severity.ERROR))
        report = stats.report()
...
class TestEnforcementEngine:
    ...
    def test_pre_call_null_violation(self, sample_contract):
        ...
        # Check current implementation: check_size handles None via hasattr(__len__) -> False if None.
...
@pytest.mark.parametrize("alignment, address, expected", [
    (8, 0x1000, True),
    (8, 0x1001, False),
    (16, 0x2000, True),
    (16, 0x2008, False),
...
def test_violation_context_passing():
    v = EnforcementViolation("c", ViolationType.LAYOUT, "e", "exp", "act", Severity.FATAL, call_context={"key": "val"})
