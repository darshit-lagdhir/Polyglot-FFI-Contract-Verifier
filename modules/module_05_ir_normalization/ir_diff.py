...
#
# File Integrity Identifier: c9aec762eeb81969
# ==============================================================================
...
class IRDiff:
    ...
    def total_changes(self) -> int:
        return len(self.breaking_changes) + len(self.compatible_changes) + len(self.neutral_changes)
...
class IRDiffComputer:
    ...
    def compute_diff(self, old_artifact: IRArtifact, new_artifact: IRArtifact) -> IRDiff:
        ...
        for eid in sorted(old_ids & new_ids):
            ...
            if changes:
                diff.modified_entities.append(
                    {"entity_id": eid, "changes": [c.to_dict() for c in changes]}
                )
    ...
    def _detect_entity_changes(self, old: IREntity, new: IREntity) -> List[Change]:
        ...
        if not isinstance(old, type(new)):
            # This should theoretically not happen if IDs are stable and based
            # on kind
            return [
                Change(
        ...
        for name in sorted(new_members.keys() - old_members.keys()):
            changes.append(
                Change(
                    ...
                    # Adding union member is often compatible if size doesn't
                    # change
                    entity_id=old.entity_id,
                )
            )
        ...
        if old.calling_convention != new.calling_convention:
            changes.append(
                Change(
                    ...
                    description=f"Calling convention changed from {old.calling_convention.value} to {
                        new.calling_convention.value
                    }",
                    abi_impact=ABIImpact.BREAKING,
                    entity_id=old.entity_id,
                )
                ...
                if op.parameter_name != np.parameter_name:
                    changes.append(
                        Change(
                            ...
                            description=f"Parameter {i} name changed from '{op.parameter_name}' to '{
                                np.parameter_name
                            }'",
        ...
        if old.is_const != new.is_const:
            changes.append(
                Change(
                    ...
                    abi_impact=ABIImpact.BREAKING,
                    # Sometimes breaking depending on platform/language
                    entity_id=old.entity_id,
                )
            )
        return changes
