...
class IREntityFactory:
    ...
    def structure_type_from_dict(data: Dict[str, Any]) -> StructureType:
        ...
        if 'padding_regions' in data:
            struct.padding_regions = [IREntityFactory.padding_from_dict(p) for p in data['padding_regions']]
        if 'metadata' in data and data['metadata']:
    ...
    def union_type_from_dict(data: Dict[str, Any]) -> UnionType:
        ...
        if 'members' in data:
            # Bypass add_member to avoid offset 0 check if data is legacy or weird, though normalized should be ok
