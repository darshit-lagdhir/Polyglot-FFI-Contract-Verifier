...
#
# File Integrity Identifier: 1fed447af7b1100d
# ==============================================================================
...
class TypeDeduplicator:
    ...
    def get_or_create_type_id(self, type_data: Dict[str, Any], converter: "TypeConverter") -> str:
        """
    ...
    def _compute_stable_structural_hash(self, type_data: Dict[str, Any]) -> str:
        ...
        if kind == "scalar":
            components.extend(
                [d.get("name", ""), str(d.get("size", 0)), str(d.get("is_signed", False))]
            )
        elif kind == "pointer":
...
class TypeConverter:
    ...
    def _convert_function_pointer(self, type_data: Dict[str, Any]) -> FunctionPointerType:
        ...
        for i, p_data in enumerate(type_data.get("parameters", [])):
            ...
            fp.add_parameter(
                ParameterEntity(
                    parameter_index=i, parameter_name=p_data.get("name"), type_reference=pt_id
                )
            )
    ...
    def _convert_union(self, type_data: Dict[str, Any]) -> UnionType:
        ...
        union = UnionType(union_name=name, size_bytes=size, alignment_bytes=align)
    ...
    def _convert_enum(self, type_data: Dict[str, Any]) -> EnumerationType:
        name = type_data.get("name", "anonymous")
        underlying = type_data.get(
            "underlying_type", {"kind": "scalar", "name": "int", "size": 4, "is_signed": True}
        )
    ...
    def _compute_padding(
        ...
        for f in sorted_fields:
            ...
            if offset > current:
                padding.append(
                    PaddingEntity(
                        byte_offset=current, size_bytes=offset - current, reason="field alignment"
                    )
                )
        ...
        if total_size > current:
            padding.append(
                PaddingEntity(
                    byte_offset=current, size_bytes=total_size - current, reason="trailing padding"
                )
            )
    ...
    def _convert_function(self, data: Dict[str, Any]) -> FunctionSymbol:
        ...
        for i, p_data in enumerate(data.get("parameters", [])):
            ...
            param = ParameterEntity(
                parameter_index=i, parameter_name=p_data.get("name"), type_reference=p_type_id
            )
            # Qualifiers
...
class Module04Bridge:
    ...
    def convert_artifact(self, artifact_path: Union[Path, str, Dict[str, Any]]) -> IRArtifact:
        ...
        artifact = IRArtifact(
            interface_unit=interface_unit, creation_timestamp=data.get("generation_timestamp")
        )
        return artifact
