...
class ValidationResult:
    ...
    def add_warning(self, code: str, message: str, clause_id: Optional[str] = None):
        """Add validation warning."""
        warning = ValidationWarning(
            warning_code=code, warning_message=message, layer=self.layer, clause_id=clause_id
        )
...
class CompleteValidationResult:
    ...
    def passed(self) -> bool:
        ...
        results = [self.schema_result, self.referential_result, self.constraint_result]
    ...
    def generate_report(self) -> str:
        ...
        if self.referential_result:
            lines.append(
                f"Referential Validation: {'PASS' if self.referential_result.passed else 'FAIL'}"
            )
        ...
        if self.constraint_result:
            lines.append(
                f"Constraint Validation: {'PASS' if self.constraint_result.passed else 'FAIL'}"
            )
...
class ConstraintValidator:
    ...
    def _validate_cross_clause_consistency(
        self, clauses: List[ContractClause], result: ValidationResult
    ):
    ...
    def _check_nullability_contradictions(
        self, clauses: List[ContractClause], result: ValidationResult
    ):
    ...
    def _check_ownership_contradictions(
        ...
        if len(ownership_clauses) > 1:
            result.add_warning(
                code="W_CONST_001", message="Multiple ownership clauses on same entity"
            )
