...
#
# File Integrity Identifier: 0d5428eace4cc31a
# ==============================================================================
...
class MigrationStep:
    ...
    def format_step(self) -> str:
        ...
        lines = [f"Change: {self.change_description}", f"Action: {self.required_action}"]
...
class AdvancedDiffResult:
    ...
    def format_summary(self) -> str:
        ...
        lines.append(
            f"Neutral Changes:    {len(self.detailed_changes) - len(breaking) - len(compatible)}"
        )
        return "\n".join(lines)
...
class NullabilityChangeAnalyzer:
    ...
    def generate_migration_step(
        self, old_c: ContractClause, new_c: ContractClause
...
class SizeChangeAnalyzer:
    ...
    def _extract_size(self, clause: ContractClause) -> Optional[int]:
        ...
        if not p:
            p = clause.get_parameter("expected_size")
        return p.value if p else None
...
class AdvancedContractDiffer:
    ...
    def compute_diff(
        ...
        for cid in basic.added_clauses:
            result.detailed_changes.append(
                DetailedClauseChange(
                    cid,
                    ChangeCategory.CLAUSE_ADDED,
                    ChangeImpact.COMPATIBLE,
                    new_clause=new_map[cid],
                    description=f"New requirement: {cid}",
                )
            )
        ...
        for cid in basic.removed_clauses:
            result.detailed_changes.append(
                DetailedClauseChange(
                    ...
                    old_clause=old_map[cid],
                    description=f"Requirement removed: {cid}",
                )
            )
        ...
        result.overall_impact = (
            ...
            else (ChangeImpact.COMPATIBLE if basic.added_clauses else ChangeImpact.NEUTRAL)
        )
    ...
    def _analyze_clausal_modification(
        self, old_c: ContractClause, new_c: ContractClause
    ) -> DetailedClauseChange:
        change = DetailedClauseChange(
            old_c.clause_id,
            ChangeCategory.PARAMETER_VALUE_CHANGED,
    ...
    def _build_migration_guide(self, result: AdvancedDiffResult) -> MigrationGuide:
        ...
        for change in result.get_breaking_changes():
            if not change.old_clause or not change.new_clause:
                continue
            if change.old_clause.clause_type == ClauseType.NULLABILITY:
                step = self.null_analyzer.generate_migration_step(
                    change.old_clause, change.new_clause
                )
                if step:
                    guide.add_step(step)
