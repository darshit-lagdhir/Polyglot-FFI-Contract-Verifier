...
from .contract_entities import (
    ContractDocument, ContractHeader, GenerationMetadata, 
    SubjectReference, SubjectKind, Severity
)
from .clause_types import (
    LayoutClause, SizeClause, AlignmentClause, NullabilityClause, 
    OwnershipClause, LifetimeClause, RelationalClause, CallingConventionClause
)
...
class GenerationConfig:
    ...
    nullable_prefixes: List[str] = field(default_factory=lambda: [
        'optional_', 'maybe_', 'nullable_', 'opt_'
    ])
    alloc_prefixes: List[str] = field(default_factory=lambda: [
        'create_', 'alloc_', 'new_', 'make_', 'build_'
    ])
    borrow_prefixes: List[str] = field(default_factory=lambda: [
        'get_', 'peek_', 'view_', 'find_', 'lookup_'
    ])
    buffer_names: List[str] = field(default_factory=lambda: [
        'buffer', 'buf', 'data', 'ptr', 'array', 'items'
    ])
    length_names: List[str] = field(default_factory=lambda: [
        'length', 'len', 'size', 'count', 'num', 'n'
    ])
...
class NullabilityClauseGenerator:
    ...
    def generate_for_parameter(self, func_name: str, p_name: str, p_id: str) -> Optional[GeneratedClause]:
...
class OwnershipClauseGenerator:
    ...
    def generate_for_return(self, function: MockIRFunction) -> Optional[GeneratedClause]:
        if not function.return_type: return None
...
__all__ = [
    'GenerationConfig', 'GeneratedClause', 'NamingPatternMatcher',
    'LayoutClauseGenerator', 'NullabilityClauseGenerator',
    'OwnershipClauseGenerator', 'RelationalClauseGenerator',
    'ContractGenerator', 'MockIRType', 'MockIRFunction'
]
