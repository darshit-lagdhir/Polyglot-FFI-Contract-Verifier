...
def generate(
    ...
    try:
        ...
        contract = generator.generate(
            ir_artifact=None,
            target_interface_id=f"ir_from_{Path(ir_artifact).stem}"
        )
...
@click.option('--filter', 'impact_filter', type=click.Choice(['breaking', 'compatible', 'neutral']), help='Filter changes by impact')
...
def diff(
    ...
    try:
        ...
        if ctx.format == 'json':
            impact_val = result.overall_impact.value if hasattr(result.overall_impact, 'value') else result.overall_impact
            output = {
    ...
    try:
        ...
        if show_header:
            ...
            for clause in contract.clauses:
                ctype = clause.clause_type.value if hasattr(clause.clause_type, 'value') else clause.clause_type
