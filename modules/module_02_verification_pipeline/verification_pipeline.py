...
#
# File Integrity Identifier: e88f0dd34c1d27fc
# ==============================================================================
...
class ArtifactValidator:
    ...
    def validate_artifact(
        artifact_path: str, expected_schema_version: Optional[str] = None
    ) -> Dict[str, Any]:
        """
...
class PipelineStage(ABC):
    ...
    def validate_preconditions(self) -> None:
        ...
        """
        artifacts_dir = self.execution_context.get("artifacts", {}).get(
    ...
    def create_provenance(self, input_artifacts: List[str]) -> ArtifactProvenance:
        ...
        for artifact_path in input_artifacts:
            if os.path.exists(artifact_path):
                input_hashes[artifact_path] = ArtifactValidator.compute_artifact_hash(
                    artifact_path
                )
...
class StageRegistry:
    ...
    def register_stage(self, stage_class: type) -> None:
        ...
        if not issubclass(stage_class, PipelineStage):
            raise ValueError(
                f"Stage class must inherit from PipelineStage: {stage_class}"
            )
...
class VerificationPipeline:
    ...
    def __init__(self, execution_context_path: str):
        ...
        # Load and validate execution context
        try:
            self.execution_context = ArtifactValidator.validate_artifact(
    ...
    def execute_full_pipeline(self) -> bool:
        ...
        for stage_class in self._resolve_execution_order():
            try:
                ...
                print(f"ERROR: {e}")
                print(
                    f"HINT: Run stage '{e.required_stage}' first to produce '{e.missing_artifact}'"
                )
                success = False
                break
                ...
                self.execution_log.log_stage_failed(stage, e)
                print(
                    f"INTERNAL ERROR: Stage '{e.stage_name}' produced invalid artifact: {e}"
                )
                print("This is a bug in the stage implementation. Please report it.")
                success = False
                break
                ...
                self.execution_log.log_stage_failed(stage, e)
                print(
                    f"UNEXPECTED ERROR in stage '{stage_class.STAGE_NAME}': {e}")
                success = False
                break
    ...
    def _resolve_execution_order(self) -> List[type]:
        ...
        return [
            self.registry.get_stage_class(name) for name in self.registry.list_stages()
        ]
    ...
    def list_stages(self) -> None:
        ...
        for stage_name in self.registry.list_stages():
            ...
            print(
                f"  Outputs: {', '.join(info['produced_outputs']) or 'None'}")
            print()
...
class InvalidStateTransitionError(PipelineError):
    ...
    def __init__(
        self, stage_name: str, current_state: StageState, attempted_state: StageState
    ):
        self.stage_name = stage_name
...
class StateMachineValidator:
    ...
    def validate_transition(
        cls, stage_name: str, current_state: StageState, new_state: StageState
    ) -> None:
        """
        ...
        if new_state not in cls.VALID_TRANSITIONS[current_state]:
            raise InvalidStateTransitionError(
                stage_name, current_state, new_state)
...
class SemanticVersion:
    ...
    def parse(cls, version_string: str) -> "SemanticVersion":
        ...
        if len(parts) != 3:
            raise ValueError(
                f"Invalid semantic version: {version_string} (expected MAJOR.MINOR.PATCH)"
            )
        ...
        try:
            ...
            raise ValueError(
                f"Invalid semantic version: {version_string} (parts must be integers)"
            )
...
class SchemaVersionValidator:
    ...
    def validate_compatibility(
        artifact_path: str, artifact_version: str, required_version: str
    ) -> None:
        """
...
class EnhancedArtifactValidator:
    ...
    def __init__(self):
        ...
        self._validation_cache: Dict[
            Tuple[str, float, int], Tuple[bool, Dict, str]
        ] = {}
    ...
    def validate_artifact(
        ...
        if os.path.exists(artifact_path):
            ...
            if cache_key in self._validation_cache:
                ...
                artifact = ArtifactValidator.validate_artifact(
                    artifact_path, expected_schema_version
                )
    ...
    def _validate_provenance_fields(
        self, artifact: Dict[str, Any], artifact_path: str
    ) -> None:
...
class DependencyGraph:
    ...
    def __init__(self, stages: List[type]):
        ...
        # stage_name → set of stages it depends on
...
class ArtifactSchema:
        ...
        for field in self.fields:
            ...
            if field.constraints:
                constraint_errors = self._check_constraints(
                    field.name, value, field.constraints
                )
    ...
    def _check_constraints(
        self, field_name: str, value: Any, constraints: Dict
    ) -> List[str]:
        ...
        if "enum" in constraints and value not in constraints["enum"]:
            errors.append(
                f"Field {field_name} not in allowed values: {constraints['enum']}"
            )
...
class SchemaRegistry:
    ...
    def get_latest_schema(self, artifact_type: ArtifactType) -> ArtifactSchema:
        ...
        latest_version = sorted(
            versions, key=lambda v: SemanticVersion.parse(v))[-1]
    ...
    def _register_builtin_schemas(self) -> None:
        ...
        execution_context_schema = ArtifactSchema(
            ...
            fields=[
                FieldSchema("platform", "object", True,
                            "Platform identification"),
                FieldSchema("compiler", "object", True,
                            "Compiler information"),
                FieldSchema("native_library", "object",
                            True, "Native library info"),
                FieldSchema(
                    "target_runtime", "object", True, "Target language runtime"
                ),
                FieldSchema(
                    "verification_config", "object", True, "Verification configuration"
                ),
                FieldSchema("provenance", "object",
                            True, "Provenance metadata"),
        ...
        pipeline_log_schema = ArtifactSchema(
            ...
            fields=[
                FieldSchema("execution_id", "string",
                            True, "Execution ID (UUID)"),
                FieldSchema(
                    "start_time", "string", True, "Pipeline start timestamp (ISO 8601)"
                ),
                FieldSchema("end_time", "string", False,
...
class ProvenanceChainValidator:
    ...
    def validate_chain(self, artifact_paths: List[str]) -> List[str]:
        ...
        for path in artifact_paths:
            try:
                artifact = self.artifact_validator.validate_artifact(
        ...
        execution_ids = set(a["provenance"]["execution_id"]
        ...
        artifacts_with_time = [
            (path, a["provenance"]["creation_timestamp"], a) for path, a in artifacts
        ]
        ...
        if self._has_cycle(dep_graph):
            errors.append(
                "Provenance chain contains cycle (artifact depends on itself)"
            )

        # Verify hash chains
        for path, artifact in artifacts:
            provenance = artifact["provenance"]
            for input_path, declared_hash in provenance[
                "input_artifact_hashes"
            ].items():
                if not os.path.exists(input_path):
                    errors.append(
                        f"{path} references missing input: {input_path}")
                    continue

                actual_hash = ArtifactValidator.compute_artifact_hash(
                    input_path)
                if actual_hash != declared_hash:
                    errors.append(
...
class StalenessDetector:
    ...
    def check_staleness(self, artifact_path: str, stage_class: type) -> StalenessStatus:
        ...
        try:
            artifact = self.artifact_validator.validate_artifact(
                artifact_path, verify_hashes=False
            )
        except Exception:
        ...
        if self.current_execution_context:
            current_exec_id = self.current_execution_context["provenance"][
                "execution_id"
            ]
...
class IncrementalPipelineExecutor:
    ...
    def execute_incremental(self, target_artifact: Optional[str] = None) -> bool:
        ...
        if target_artifact:
            execution_order = self._prune_to_target(
                execution_order, target_artifact, dep_graph
            )
        ...
        for stage_name in execution_order:
            ...
            artifacts_dir = self.pipeline.execution_context["artifacts"][
                "working_directory"
            ]
            artifact_paths = [
            ...
            # Execute stage
            print(
                f"▶️  Running {stage_name} (artifacts are {staleness_statuses[0].value})"
            )
...
def initialize_libclang() -> bool:
    ...
    try:
        ...
        for file in os.listdir(native_dir):
            if file.startswith("libclang") and (
                file.endswith(".dll") or file.endswith(
                    ".so") or file.endswith(".dylib")
            ):
...
class TypeExtractor:
    ...
    def extract_type(clang_type) -> Dict[str, Any]:
        ...
        if kind in [
            ...
            type_info["pointee"] = TypeExtractor.extract_type(
                clang_type.get_pointee())

        elif kind == clang.TypeKind.CONSTANTARRAY:
            type_info["kind"] = "array"
            type_info["element_type"] = TypeExtractor.extract_type(
            ...
            type_info["kind"] = (
                "struct" if decl.kind == clang.IDEKind.STRUCT_DECL else "union"
            )
            type_info["name"] = clang_type.spelling
            ...
            type_info["name"] = clang_type.spelling
            type_info["underlying_type"] = TypeExtractor.extract_type(
                clang_type.get_canonical()
            )
...
class StructLayoutExtractor:
    ...
    def extract_layout(cursor) -> Dict[str, Any]:
        ...
        if is_union:
            ...
            layout["fields"] = StructLayoutExtractor._insert_padding(
                declared_fields, layout["size_bytes"], layout["alignment_bytes"]
            )
    ...
    @staticmethod
    def _insert_padding(
        fields: List[Dict], total_size: int, alignment: int
    ) -> List[Dict]:
        """
...
class NativeInterfaceIngestionStage(PipelineStage):
    ...
    def _execute_impl(self) -> None:
        ...
        if tu.diagnostics:
            ...
            if errors:
                error_msgs = "\n".join(
                    [f"{d.location}: {d.spelling}" for d in errors])
                raise StageError(
        ...
        for cursor in tu.cursor.walk_preorder():
            ...
            if cursor.kind == clang.IDEKind.FUNCTION_DECL:
                ...
                if (
                    ...
                    structures.append(
                        StructLayoutExtractor.extract_layout(cursor))
    ...
    def _extract_typedef(self, cursor) -> Dict[str, Any]:
        ...
        return {
            "name": cursor.spelling,
            "underlying_type": TypeExtractor.extract_type(
...
class TypedefResolver:
    ...
    def __init__(self):
        # typedef_name → full typedef info
        self._typedef_map: Dict[str, Dict] = {}
    ...
    def resolve(self, type_info: Dict[str, Any]) -> Dict[str, Any]:
        ...
        while current.get("kind") == "typedef":
            ...
            if typedef_name in visited:
                ...
                raise StageError(
                    f"Circular typedef detected: {cycle}", stage_name="ir_normalization"
                )
...
class TypeNormalizer:
    ...
    def normalize_type(
        self, native_type: Dict[str, Any], resolve_typedefs: bool = True
    ) -> str:
        """
        ...
        if resolve_typedefs and native_type.get("kind") == "typedef":
            ...
            underlying_id = self.normalize_type(
                resolved, resolve_typedefs=False)
...
class IRNormalizationStage(PipelineStage):
    ...
    def _execute_impl(self) -> None:
        ...
        for struct in native_interface.get("structures", []):
            normalized_struct = self._normalize_structure(
        ...
        platform_info = {
            ...
            "alignment_rules": (
                "msvc"
                if self.execution_context["platform"]["os_name"] == "Windows"
                else "gcc"
            ),
        }
    ...
    def _normalize_structure(self, struct: Dict, normalizer: TypeNormalizer) -> Dict:
        ...
        for field in struct.get("fields", []):
            ...
            if "is_bitfield" in field:
                ...
                normalized_field["bitfield_offset"] = field.get(
                    "bitfield_offset", 0)
...
class NamingPatternAnalyzer:
    ...
    def suggests_nullable(name: str) -> bool:
        ...
        return any(
            pattern in name_lower for pattern in NamingPatternAnalyzer.NULLABLE_PATTERNS
        )
    ...
    def suggests_ownership_transfer_out(name: str) -> bool:
        ...
        name_lower = name.lower()
        return any(
            name_lower.startswith(pattern)
            for pattern in NamingPatternAnalyzer.CREATE_PATTERNS
        )
    ...
    def suggests_ownership_transfer_in(name: str) -> bool:
        ...
        return any(
            name_lower.startswith(pattern)
            for pattern in NamingPatternAnalyzer.DESTROY_PATTERNS
        )
    ...
    def suggests_size_parameter(name: str) -> bool:
        ...
        name_lower = name.lower()
        return any(
            pattern in name_lower for pattern in NamingPatternAnalyzer.SIZE_PATTERNS
        )
    ...
    def suggests_buffer_parameter(name: str) -> bool:
        ...
        name_lower = name.lower()
        return any(
            pattern in name_lower for pattern in NamingPatternAnalyzer.BUFFER_PATTERNS
        )
...
class ConstraintSynthesizer:
    ...
    def synthesize_function_constraints(
        ...
        return_constraints = self._synthesize_return_constraints(
            func_name, function)
        constraints.extend(return_constraints)

        # Synthesize calling convention constraint
        conv_constraint = self._synthesize_calling_convention_constraint(
            func_name, function
        )
        if conv_constraint:
    ...
    def _synthesize_parameter_constraints(
        self, func_name: str, param: Dict[str, Any], all_params: List[Dict[str, Any]]
    ) -> List[Constraint]:
        ...
        if param_type.get("kind") == "pointer":
            ...
            null_constraint = self._infer_nullability(
                func_name, param_name, param, param_type
            )
            constraints.append(null_constraint)

            # Check for buffer-length relationship
            buffer_constraint = self._infer_buffer_size(
                func_name, param, all_params)
            ...
            if self._is_output_parameter(param_type):
                output_constraint = self._create_output_parameter_constraint(
    ...
    def _infer_nullability(
        self, func_name: str, param_name: str, param: Dict, param_type: Dict
    ) -> Constraint:
        ...
        if NamingPatternAnalyzer.suggests_nullable(param_name):
            ...
            evidence = [
                "No naming hints for nullability",
                "Conservative default: non-null",
            ]
    ...
    def _infer_buffer_size(
        self, func_name: str, param: Dict, all_params: List[Dict]
    ) -> Optional[Constraint]:
        ...
        for other_param in all_params:
            ...
            if other_type.get("kind") == "primitive":
                if "int" in other_type.get("name", "").lower():
                    # Check naming
                    if NamingPatternAnalyzer.suggests_size_parameter(
                        other_param["name"]
                    ):
                        confidence = 0.85
                        evidence = [
            ...
            if pointee_id:
                ...
                if pointee.get("name") == "char" and param_type.get("is_const"):
                    ...
                    return Constraint(
                        ...
                        target=f"param_{param_name}",
                        confidence=0.7,
                        rationale="const char* parameter without size likely null-terminated string",
    ...
    def _create_output_parameter_constraint(
        self, func_name: str, param_name: str
    ) -> Constraint:
        """Create constraint for output parameter."""
        self.constraint_counter += 1
        constraint_id = (
            f"{func_name}_OUTPUT_PARAMETER_{param_name}_{self.constraint_counter}"
        )
    ...
    def _create_ownership_transferred_in_constraint(
        ...
        self.constraint_counter += 1
        constraint_id = (
            f"{func_name}_OWNERSHIP_IN_{param_name}_{self.constraint_counter}"
        )
    ...
    def _synthesize_return_constraints(
        self, func_name: str, function: Dict
    ) -> List[Constraint]:
        ...
        if return_type.get("kind") == "pointer":
            ...
            if NamingPatternAnalyzer.suggests_ownership_transfer_out(func_name):
                self.constraint_counter += 1
                constraint_id = (
                    f"{func_name}_OWNERSHIP_OUT_return_{self.constraint_counter}"
                )
    ...
    def _synthesize_calling_convention_constraint(
        ...
        if platform.get("pointer_size") == 8:  # x64
            expected = "win64" if platform.get(
                "alignment_rules") == "msvc" else "sysv"
        else:  # x86
            expected = "cdecl"
...
class CheckGenerator:
    ...
    def generate_check_function(
        constraint: Dict, function_name: str, type_registry: Dict
    ) -> str:
        """
        ...
        if constraint_type == "non_null":
            CheckGenerator._generate_non_null_check(
                gen, constraint, function_name)

        elif constraint_type == "buffer_size":
            CheckGenerator._generate_buffer_size_check(
                gen, constraint, function_name)

        elif constraint_type == "null_terminated":
            CheckGenerator._generate_null_terminated_check(
                gen, constraint, function_name
            )
    ...
    @staticmethod
    def _generate_buffer_size_check(
        gen: CodeGenerator, constraint: Dict, func_name: str
    ):
        ...
        gen.add_line(f'constraint_id="{constraint_id}",')
        gen.add_line(
            f"message=f\"Buffer '{target}' size {{len({target})}} < required {{{related}}}\","
        )
        gen.add_line(f'function="{func_name}",')
        gen.add_line(f'parameter="{target}"')
        gen.dedent()
    ...
    @staticmethod
    def _generate_null_terminated_check(
        gen: CodeGenerator, constraint: Dict, func_name: str
    ):
...
class AdapterGenerator:
    ...
    def generate_adapter_module(self) -> str:
        ...
        gen.add_line(
            '_lib = ctypes.CDLL("library.dll")  # TODO: Use actual library path'
        )
        gen.add_line()
    ...
    def _generate_function_wrapper(self, func_contract: Dict) -> str:
        ...
        gen.add_line('"""')
        gen.add_line(
            f"Wrapper for: {func_contract.get('signature', func_name)}")
        gen.add_line()
        gen.add_line("Enforced constraints:")
        for constraint in func_contract.get("constraints", []):
            gen.add_line(
                f"- {constraint['constraint_id']}: {constraint['type']}")
        gen.add_line('"""')
        ...
        for constraint in func_contract.get("constraints", []):
            ...
            if constraint_type in ["non_null", "null_terminated"]:
                gen.add_line(
                    f"_check_{constraint_type.upper()}_{target}({target})")
            elif constraint_type == "buffer_size":
                related = constraint.get(
                    "related_target", "").replace("param_", "")
                gen.add_line(
                    f"_check_BUFFER_SIZE_{target}({target}, {related})")
...
class TestCaseGenerator:
    ...
    def _generate_positive_test(self, func_contract: Dict, func_ir: Dict) -> Dict:
        ...
        return {
            ...
            "constraints_exercised": [
                c["constraint_id"] for c in func_contract.get("constraints", [])
            ],
            "inputs": inputs,
            "expected_outcome": {"type": "success", "no_violations": True},
        }

    def _generate_valid_input_for_type(
        self, param_type: Dict, param_name: str, func_contract: Dict
    ) -> Dict:
        ...
        if kind == "primitive":
            ...
            if "int" in type_name:
                return {"type": "int", "value": 42, "generator": "fixed_valid_int"}
            ...
            size_param = self._find_buffer_size_param(
                param_name, func_contract)
        ...
        # Default
        return {"type": "unknown", "value": None, "generator": "none"}

    def _find_buffer_size_param(
        self, buffer_param: str, func_contract: Dict
    ) -> Optional[str]:
    ...
    def _generate_negative_tests(
        self, func_contract: Dict, func_ir: Dict
    ) -> List[Dict]:
    ...
    def _generate_inputs_violating_constraint(
        self, constraint: Dict, func_contract: Dict, func_ir: Dict
    ) -> Dict:
        ...
        for param in func_ir.get("parameters", []):
            ...
            if param_type.get("kind") == "primitive":
                ...
                if "int" in type_name:
                    ...
                    self.test_counter += 1
                    test_id = (
                        f"test_{func_contract['name']}_{self.test_counter:03d}_bnd"
                    )
                    ...
                    for p in func_ir.get("parameters", []):
                        if p["name"] == param["name"]:
                            ...
                            # Use valid value
                            p_type = self.ir["type_registry"].get(
                                p["type_id"], {})
                    ...
                    tests.append(
                        {
                            ...
                            "constraints_exercised": [
                                c["constraint_id"]
                                for c in func_contract.get("constraints", [])
                            ],
                            "inputs": inputs,
                            "expected_outcome": {
                                "type": "success",
...
class CoverageAnalyzer:
    ...
    def analyze_coverage(test_cases: List[Dict], contract: Dict) -> Dict:
        ...
        coverage_pct = (
            (len(exercised_constraints) / len(all_constraints) * 100)
            if all_constraints
            else 100.0
        )
...
class TestPlanGenerationStage(PipelineStage):
    ...
    def _execute_impl(self) -> None:
        ...
        for func_contract in contract.get("functions", []):
            test_cases = generator.generate_test_cases_for_function(
                func_contract)
        ...
        for test in all_test_cases:
            ...
            tests_by_category[category] = tests_by_category.get(
                category, 0) + 1
        ...
        metadata = {
            ...
            # Rough estimate
            "estimated_execution_time_seconds": len(all_test_cases) * 2,
            "deterministic": True,
            "reproducible": True,
        }
        ...
        if coverage["uncovered_constraints"]:
            print(
                f"Warning: {len(coverage['uncovered_constraints'])} constraints not covered"
            )
...
class InputInstantiator:
    ...
    def instantiate_all(inputs: Dict[str, Dict]) -> Dict[str, Any]:
        ...
        for param_name, input_spec in inputs.items():
            instantiated[param_name] = InputInstantiator.instantiate(
                input_spec)
        ...
        if expected_type == "success":
            if actual_type == "success" and actual.get("no_violations"):
                ...
                if actual_type == "contract_violation":
                    ...
                    reason += (
                        f" ({actual.get('exception_type')}: {actual.get('message')})"
                    )
                return ("FAIL", reason)
...
class TestExecutor:
    ...
    def _load_adapter(self):
        ...
        try:
            spec = importlib.util.spec_from_file_location(
                "adapter", self.adapter_module_path
            )
    ...
    def execute_test(self, test_case: Dict) -> Dict:
        ...
        # Instantiate inputs
        try:
            inputs = InputInstantiator.instantiate_all(
    ...
    def _invoke_adapter(self, function, inputs: Dict) -> Dict:
        ...
        try:
            ...
            # Success outcome
            return {"type": "success", "return_value": result, "no_violations": True}
    ...
    def _generate_diagnostic(
        self, test_case: Dict, expected: Dict, actual: Dict
    ) -> str:
...
class ExecutionSummarizer:
    ...
    def summarize(test_results: List[Dict]) -> Dict:
        ...
        passed = sum(
            1 for r in test_results if r["validation_result"] == "PASS")
        failed = sum(
            1 for r in test_results if r["validation_result"] == "FAIL")
        errors = sum(
            1 for r in test_results if r["validation_result"] == "ERROR")

        pass_rate = (passed / total * 100) if total > 0 else 0.0

        # Total execution time
        total_time = sum(r.get("execution_time_ms", 0)
                         for r in test_results) / 1000.0
...
class VerificationExecutionStage(PipelineStage):
    ...
    def _execute_impl(self) -> None:
        ...
        artifacts_dir = self.execution_context["artifacts"]["working_directory"]
        test_plan_path = os.path.join(artifacts_dir, "test_plan.json")
        adapter_metadata_path = os.path.join(
            artifacts_dir, "adapter_metadata.json")
        ...
        for i, test_case in enumerate(test_cases, 1):
            try:
                ...
                print(
                    f"  [{i}/{len(test_cases)}] {status} {test_case['test_id']}")

            except Exception as e:
                # Catch unexpected errors
                print(
                    f"  [{i}/{len(test_cases)}] ✗ {test_case['test_id']} (ERROR)")
                test_results.append(
                    {
                        "test_id": test_case["test_id"],
                        "validation_result": "ERROR",
        ...
        provenance = self.create_provenance(
            [test_plan_path, adapter_metadata_path])
        ...
        print(
            f"Passed: {summary['tests_passed']} ({summary['pass_rate']:.1f}%)")
        print(f"Failed: {summary['tests_failed']}")
        print(f"Errors: {summary['tests_errored']}")
        print(
            f"Execution time: {summary['execution_time_total_seconds']:.2f}s")
        print("=" * 60)
        ...
        if failures:
            ...
            for failure in failures[:10]:  # Show first 10
                print(
                    f"  - {failure['test_id']}: {failure.get('failure_reason', 'Unknown')}"
                )
...
class FailureClassifier:
    ...
    def classify(test_result: Dict, test_case: Dict) -> Dict:
        ...
        if (
            expected.get("type") == "contract_violation"
            and actual.get("type") == "success"
        ):
            return {
        ...
        if (
            expected.get("type") == "success"
            and actual.get("type") == "contract_violation"
            and category_name == "positive"
        ):
            return {
        ...
        if expected.get("type") == "success" and actual.get("type") in [
            "crash",
...
class RemediationGenerator:
    ...
    def generate(classification: Dict, test_result: Dict, test_case: Dict) -> List[str]:
        ...
        if category == FailureCategory.UNCAUGHT_VIOLATION:
            constraint_id = test_result["expected_outcome"].get(
                "expected_constraint_id", "unknown"
            )
            return [
            ...
            constraint_id = test_result["actual_outcome"].get(
...
class HTMLReportGenerator:
    ...
    def generate(execution_log: Dict, diagnostics: Dict) -> str:
        ...
        html.append(
            f"<p>Tests Passed: {summary.get('tests_passed', 0)}/{summary.get('total_tests', 0)}</p>"
        )

        critical_count = diagnostics.get(
            "severity_counts", {}).get("CRITICAL", 0)
        if critical_count > 0:
            html.append(
                f"<p class='critical'>⚠ {critical_count} CRITICAL issues</p>")
        ...
        if failures:
            ...
            for failure in failures:
                ...
                html.append(
                    f"<h3>{test_id} <span class='severity {severity.lower()}'>{severity}</span></h3>"
                )
        ...
        html.append("<hr>")
        html.append(
            f"<p class='footer'>Generated: {datetime.now(timezone.utc).isoformat()}</p>"
        )
        html.append("</body>")
        html.append("</html>")
...
class MarkdownReportGenerator:
    ...
    def generate(execution_log: Dict, diagnostics: Dict) -> str:
        ...
        md.append(
            f"- **Execution Time:** {summary.get('execution_time_total_seconds', 0):.2f}s"
        )
        ...
        for category, stats in summary.get("tests_by_category", {}).items():
            md.append(
                f"| {category} | {stats['total']} | {stats['passed']} | {stats['failed']} |"
            )
    ...
    def _execute_impl(self) -> None:
        ...
        for result in test_results:
            if result.get("validation_result") != "PASS":
                ...
                remediation = RemediationGenerator.generate(
                    classification, result, test_case
                )
        ...
        provenance = self.create_provenance(
            [execution_log_path, test_plan_path, contract_path]
        )
        ...
        # Generate Markdown report
        md_report = MarkdownReportGenerator.generate(
            execution_log, diagnostics)
...
class CompletePipeline:
    ...
    def __init__(
        self, header_path: str, library_path: str, output_dir: str = "artifacts"
    ):
        """
        ...
        context_data = {
            ...
            "artifacts": {
                ...
                "execution_context_path": os.path.join(
                    self.output_dir, "execution_context.json"
                ),
            },
        }
    ...
    def _build_result(self, success: bool) -> VerificationResult:
        ...
        execution_log_path = os.path.join(
            self.output_dir, "execution_log.json")
        if os.path.exists(execution_log_path):
            ...
            diagnostics_path = os.path.join(
                self.output_dir, "diagnostics.json")
            critical_issues = []
...
def cli_main():
    ...
    verify_cmd = subparsers.add_parser(
        "verify", help="Run complete verification")
    ...
    list_cmd = subparsers.add_parser(
        "list-stages", help="List available pipeline stages"
    )
    ...
    incremental_cmd = subparsers.add_parser(
        "run-incremental", help="Run pipeline incrementally"
    )
    incremental_cmd.add_argument(
        "--context", required=True, help="Execution context path"
    )
    incremental_cmd.add_argument("--target", help="Target artifact to produce")

    staleness_cmd = subparsers.add_parser(
        "check-staleness", help="Check artifact staleness"
    )
    ...
    if args.command == "verify":
        ...
        try:
            ...
            staleness_detector.set_current_execution_context(
                pipeline.execution_context)

            executor = IncrementalPipelineExecutor(
                pipeline, staleness_detector)
...
class CacheManager:
    ...
    def _init_database(self):
        ...
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_stage_name ON cache_entries(stage_name)"
        )
        cursor.execute(
    ...
    def lookup(
        self, stage_name: str, stage_version: str, inputs: Dict[str, str]
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        with self.lock:
            ...
            cursor.execute(
                "SELECT stage_name, COUNT(*), SUM(hits) FROM cache_entries GROUP BY stage_name"
            )
            by_stage = {
                row[0]: {"entries": row[1], "hits": row[2] or 0}
...
class ParallelPipelineExecutor:
    ...
    def execute_parallel(self) -> bool:
        ...
        for level_num, level_stages in enumerate(levels):
            print(
                f"Executing level {level_num + 1}/{len(levels)}: {len(level_stages)} stage(s)"
            )
...
class PerformanceProfiler:
    ...
    def generate_report(self) -> str:
        ...
        lines.append("=" * 80)
        lines.append(
            f"{'Stage':<30} {'Time':>10} {'CPU':>10} {'I/O':>10} {'Memory':>12}"
        )
        lines.append("-" * 80)
    ...
    def _execute_parallel(self, verbose: bool) -> VerificationResult:
        ...
        try:
            ...
            executor = ParallelPipelineExecutor(
                self.pipeline, self.max_workers)
    ...
    def get_applicable_rules(self, context: Dict) -> List[Dict]:
        ...
        for rule_id, rule_info in self.rules.items():
            ...
            if heuristic is None or heuristic(context):
                applicable.append({"rule_id": rule_id, **rule_info})
...
class ExtensiblePipeline(OptimizedCompletePipeline):
    ...
    def register_custom_rule(
        ...
        self.rule_registry.register(
            rule_id, constraint_class, synthesis_heuristic)

    def execute(self, verbose: bool = True) -> VerificationResult:
        """Execute with hooks."""
        # Execute pre-pipeline hooks
        context = HookContext(
            execution_id=str(uuid.uuid4()), pipeline=self, metadata={}
        )
        ...
        try:
            ...
            self.hook_manager.execute(
                HookPoints.POST_PIPELINE, context, result=result)
            ...
            # Execute error hooks
            self.hook_manager.execute(
                HookPoints.PIPELINE_ERROR, context, error=e)
            raise
...
def verify_extensible(
    ...
    **kwargs,
    ...
    pipeline = ExtensiblePipeline(
        header_path, library_path, output_dir, **kwargs)
