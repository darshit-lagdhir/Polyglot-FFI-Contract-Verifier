...
`ExecutionContext (Phase 1)` -> `Native Interface Ingestion (Phase 2)` -> **`IR Normalization (Phase 3)`** -> `Contract Synthesis (Phase 4)`
...
### Structs
Struct layouts extracted in Phase 2 are preserved exactly (including explicit padding fields). The normalization process replaces the deep, inline type definitions for fields with flat `type_id` references to the Type Registry.
...
- **Precondition Errors**: If `native_interface.json` is missing, the normalizer reports that Phase 2 must be run first.
